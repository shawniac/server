...
Stopping slave I/O thread due to out-of-memory error from master");
      ...
      */
      ...
      /* Control the binlog apply speed to master
      ...
      */
      ...
      if (apply_speed_limit_in_bytes)
      {
        /* Prevent the tokenamount become a large value,
           for example, the IO thread doesn't work for a long time
        */
        if (tokenamount > apply_speed_limit_in_bytes * 2)
        {
          lastchecktime = my_hrtime().val;
          tokenamount = apply_speed_limit_in_bytes * 2;
        }

        do
        {
          ulonglong currenttime = my_hrtime().val;
          tokenamount += (currenttime - lastchecktime) * apply_speed_limit_in_bytes / (1000*1000);
          lastchecktime = currenttime;
          if(tokenamount < network_read_len)
          {
            ulonglong duration =1000ULL*1000 * (network_read_len - tokenamount) / apply_speed_limit_in_bytes;
            time_t second_time = (time_t)(duration / (1000 * 1000));
            uint micro_time = duration % (1000 * 1000);
